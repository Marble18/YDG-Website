...
- 2026-09-07: Extended PR #21 with a Material 3 Expressive, light-only Customer Dashboard presentation while preserving the soft pink/coral token palette. Customer search now sits responsively between the logo and account menu and still drives the existing debounced database query. Categories retain database filtering, pagination reset and sticky horizontal navigation. Live catalogue cards use uncropped contained images, two columns on mobile, pill unit/quantity controls and 44 px touch targets; a mobile safe-area-aware floating Cart opens the existing Supabase cart and mirrors its item count. The Pcs/Box form and card/cart preview remain a deliberately non-persisting UI prototype: the production single-unit Product, cart, checkout, order, inventory, voucher and RLS/API contracts are unchanged. Automated syntax/build/diff and responsive visual checks are required; authenticated Deploy Preview checks remain for search/filter/load-more, cart persistence, lightbox, mobile layouts and Owner regressions.
