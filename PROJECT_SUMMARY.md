...
- 2026-09-16: Customer catalogue card information cleanup on `codex/customer-card-info-layout`. Unit price is now 18–24 px with clearer spacing; pcs shows unit price/minimum without a calculation. Box shows `1 box = N pcs` and a polite live quantity × box price = total summary. Customer-only styles keep action buttons aligned, wrap long text and retain 44 px unit/quantity/cart touch targets. No service, database, owner/staff, product form, order, voucher or export contract changes. Local browser checks at 320/375/412/768/1280 px passed: mobile two columns, no horizontal overflow, price size, action alignment, box initial/toggle/typed/stepper totals, pcs hidden state, zero price, minimum clamp, unit-specific cart RPC payloads and separate cart update/removal with mocked responses, landscape/portrait/square containment, lightbox Escape/focus return and keyboard unit selection. JavaScript syntax and diff whitespace checks passed. Remaining Deploy Preview manual tests: authenticated customer cart persistence after refresh/re-login, physical phone touch/layout, catalogue filtering/pagination and owner/staff smoke checks. Merge/production release awaits approval.
