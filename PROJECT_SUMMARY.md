...
- Hosted browser connection: same-origin `/supabase` allowlisted proxy for Auth, REST, Storage and Edge Functions; localhost/file development falls back to the direct project URL
...
- Realtime/WebSocket and GraphQL routes are not proxied because this application does not use them. Netlify external rewrites have a 26-second timeout, so long-running database/Storage backup operations require explicit Deploy Preview regression testing.
