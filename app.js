(function () {
  ...
  function renderCustomerProducts() {
    ...
    grid.innerHTML = products.length ? products.map(function (product) {
      ...
      var showBoxBreakdown = product.salesMode === 'pcs_and_box' && defaultUnit === 'box';
    ...
    function refreshCard(productId, selectedUnit, resetQuantity) {
      ...
      var equivalent = card.querySelector('[data-card-equivalent]'); equivalent.hidden = selectedUnit !== 'box'; equivalent.textContent = selectedUnit === 'box' ? '1 box = ' + product.piecesPerBox + ' pcs' : '';
      ...
      var showBoxBreakdown = product.salesMode === 'pcs_and_box' && selectedUnit === 'box';
      total.hidden = !showBoxBreakdown;
      total.textContent = showBoxBreakdown ? customerBoxPriceBreakdown(product) : '';
    }
  ...
  function renderCustomerMenu() {
    document.getElementById('customer-menu-root').innerHTML = '<div class="customer-menu-backdrop" id="close-customer-menu"></div><aside class="customer-menu-panel"><div class="customer-menu-head"><div><p class="eyebrow">Customer menu</p><h2>' + esc(currentUser.name) + '</h2></div><button class="icon-btn" id="close-customer-menu-button">×</button></div><button class="customer-menu-item danger" id="customer-menu-logout"><span>Log out</span><small>Sign out of this customer account</small></button></aside>';
  ...
  function settingsPage() {
    ...
    return '<div class="page-heading"><div><p class="eyebrow">Shop controls</p><h1>Settings</h1><p>Configure site maintenance and your customer voucher style.</p></div></div><section class="admin-grid"><form class="panel" id="site-settings"><h2 class="panel-title">Site maintenance</h2><p class="subtext">Customers cannot log in while Under Maintenance is enabled. Owner accounts can always access settings.</p><label class="field"><span><input name="maintenance" type="checkbox" ' + (s.maintenanceMode ? 'checked' : '') + '> Enable Under Maintenance</span></label><div class="two-button"><button class="primary" type="submit">Save site settings</button></div></form></section><section class="panel voucher-settings"><form id="voucher-settings"><div class="page-heading"><div><p class="eyebrow">Voucher design</p><h1>Customize & preview</h1><p>Customers can view and print this voucher at Ready to Ship.</p></div><button class="primary" type="submit">Save voucher style</button></div><div class="form-grid"><label class="field full-field">Voucher title<input name="title" id="voucher-title" value="' + esc(s.voucher.title) + '"></label><label class="field">Accent colour<input name="color" id="voucher-color" type="color" value="' + esc(s.voucher.accentColor) + '"></label><label class="field full-field">Footer message<input name="footer" id="voucher-footer" value="' + esc(s.voucher.footer) + '"></label></div></form><div id="voucher-preview">' + preview + '</div></section>';
  }
  ...
  function renderDatabaseCategoryAdjust() {
    ...
    modal('<form id="category-form"><div class="modal-head"><div><p class="eyebrow">Pricing tool</p><h2>Adjust category prices</h2></div><button class="icon-btn" id="close-modal" type="button">×</button></div><p class="subtext">Update every active product in one category with one protected database operation.</p><label class="field">Category<select name="categoryId" required>' + categories.map(function (category) { return '<option value="' + category.id + '">' + esc(category.name) + '</option>'; }).join('') + '</select></label><label class="field">Percentage change<input name="percentage" type="number" min="-100" max="10000" step="any" required placeholder="Example: 5.5 or -2.5"></label><p class="photo-help">5.5 increases by 5.5%. -2.5 reduces by 2.5%. The percentage is not rounded. Prices are rounded to the nearest whole MMK.</p><div id="category-price-preview"></div><p class="export-status" id="category-adjust-status" aria-live="polite"></p><div class="two-button"><button class="primary" id="apply-category-adjust" type="submit">Preview price change</button></div></form>');
