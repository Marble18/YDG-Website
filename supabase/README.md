...
PR #25 replaces the timeout-prone backup creation path with stateless bounded reads. Database pages contain at most 200 rows with stable `id` ordering, Storage directory pages contain at most 100 entries, and object bytes are fetched one approved exact path per request. The browser verifies two complete passes, builds private ZIP parts locally (at most four files / 8 MB source bytes), verifies each saved part, and writes a completion index only after the final manifest matches. Same-tab retry resumes verified parts; refresh or sign-out intentionally clears memory-only progress. This path creates no restore plan or backup-job database row and requires no migration. Existing restore validation/confirmation remains separate and unchanged.
